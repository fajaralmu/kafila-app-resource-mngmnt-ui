...
const DataTableHeader = (
  ...
  return items.map(item => {
    ...
    const value = name && filter[name] ? decodeURIComponent(filter[name]) : '';
    return (
      <th key={"th-item-" + randomString(3)} className={className}>
        ...
        {item.filterable && item.name &&
          <input id={`input-filter-${item.name}`} className="form-control form-control-sm" value={value} onChange={_onChange} />
        }
      </th>
    )
  })
