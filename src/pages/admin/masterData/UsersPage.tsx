...
class UsersPage extends BaseMasterDataPage<User, BaseProps, State> {
  ...
  getDataTableHeaderVals = () => {
    return [
      ...
      new DataTableHeaderValue("authorities.name", "Role"),
                  ...
                  {items.map((item, i) => {
                    return (
                      <tr key={"user-" + item.id}>
                        ...
                        <td>{item.gender}</td>
                        <td>
                          {item.authorities.map(a => <div key={`${item.id}-${a.name}`} className="badge bg-success me-1">{a.name?.toLocaleLowerCase()}</div>)}
                        </td>
